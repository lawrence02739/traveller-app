...
export const FlightSearchPage = () => {
  return (
    <MainLayout>
      ...
      <div className="relative h-[280px] w-full bg-[var(--color-primary)] transition-colors duration-300">
        <div className="mx-auto flex max-w-lg justify-center gap-1 pt-8 px-4">
          <div className="flex flex-wrap sm:flex-nowrap items-center rounded-lg bg-black/10 p-1 backdrop-blur-sm overflow-x-auto w-full sm:w-auto">
            {NAV_SERVICES.map((srv) => (
              <button
                key={srv.label}
                className={`flex flex-col flex-1 sm:flex-none items-center justify-center gap-1 rounded-md px-2 sm:px-4 py-2 transition-all min-w-[60px] ${
                  srv.active ? 'bg-[var(--color-primary-soft)] text-[var(--color-primary-strong)] shadow-sm' : 'text-white/90 hover:bg-white/10'
                }`}
              >
